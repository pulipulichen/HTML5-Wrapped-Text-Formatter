...
c=[];console.log(d);var l=2;1===d.length%2&&(l=1);for(var h=0,r=d.length;h<r;h+=l){var e=d[h];2===l&&(e=e+", "+d[h+1]);console.log(e);var e=e.split("&").join(""),m=e.indexOf(", ");if(-1<m)var t=e.substr(0,m).trim(),e=e.slice(m+1,e.length).trim()+" "+t;c.push(e)}a=c.join("\n")}var d=a,a=encodeURIComponent(a),p="https://translate.google.com.tw/#"+b.source_lang.value+"/"+b.target_lang.value+"/",a=p+a,f=b.output_style.value;if(b.test_output.checked){var u=$(".google_trans_20140526 .test-output").show(),
g=$("<div class='test-div panel panel-success'></div>").prependTo(u),n=new Date,v=n.getHours(),w=n.getMinutes(),x=n.getSeconds(),f=d,f=_googleTransUtils.str_replace("\n"," ",f);$("<div class='animated panel-heading togglable'></div>").html('<span class="glyphicon glyphicon-chevron-up" style="float:right"></span><span class="glyphicon glyphicon-chevron-down" style="float:right"></span><div class=\'heading-text\'><strong>'+v+":"+w+":"+x+"</strong> "+f+"</div> ").click(function(){_googleTransUtils.toggle_panel(this)}).appendTo(g);
_googleTransUtils.toggle_panel($(".google_trans_20140526 .input-div .panel-heading.togglable"));var y=$("<button type='button'></button>").html("\u7ffb\u8b6f").attr("pulipuli_base_url",p).click(function(){var a=$(this).attr("pulipuli_base_url")+$(this).next().val();window.open(a,"_blank","height=600,width=800,scrollbars=no")}).appendTo(g),k=$("<textarea class='animated source panel-body'></textarea>").val(d).click(function(){this.select()}).appendTo(g),z=$("<textarea class='animated source panel-body note'></textarea>").click(function(){this.select()}).appendTo(g);
"submit_authors"===$(b).attr("submit_type")&&g.addClass("submit_authors");var A=function(){$(".input-div .panel-body").slideUp()};$(function(){k.autosize();z.autosize();k.focus();k.click();k.click(A);$(".google-trans-title").hide()});b.window_open_output.checked&&setTimeout(function(){return y.click()},500)}else if("blank"===f)window.open(a,"_blank");else if("iframe"===f){var B=b.subject.value,C=(new Date).getTime();_googleTransUtils.create_iframe(a,C+" "+B).prependTo(".google_trans_20140526 .output")}}catch(q){console.log(q)}$(b.source).val("");
