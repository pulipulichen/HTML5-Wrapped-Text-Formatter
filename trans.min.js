...
a=a.split('." ').join('." \n\n'),a=a.split('." \n\n(').join('." ('),a=a.split("\u3002\u300d ").join("\u3002\u300d \n\n"),a=a.split(".' ").join(".' \n\n"),a=a.split("; ").join("; \n\n"),a=a.replace(/\d; \n\n/g,function(a){return a.split("\n").join("")}),a=a.split("\uff1b ").join("\uff1b \n\n"),a=_googleTransUtils.str_replace("\uff1b","\uff1b\n\n",a),a=_googleTransUtils.str_replace("\uff08 "," (",a),a=_googleTransUtils.str_replace("  ( "," (",a),a=_googleTransUtils.str_replace(" \uff09",") ",a),a=_googleTransUtils.str_replace(")  ",
...
a=$.trim(a);if("submit_authors"===$(b).attr("submit_type")){-1<a.indexOf(" (")&&(a=a.substr(0,a.indexOf(" (")).trim());var a=a.split(" and ").join(", "),d=a.split(","),c=[];console.log(d);var e=2;1===d.length%2&&(e=1);for(var g=0,w=d.length;g<w;g+=e){var f=d[g];2===e&&(f=f+", "+d[g+1]);var f=f.split("&").join(""),n=f.indexOf(", ");if(-1<n)var x=f.substr(0,n).trim(),y=f.slice(n+1,f.length).trim(),f=x+"\t"+y;c.push(f)}a=c.join("\n")}b.trim_citation.checked&&(a=a.replace(/\[.*?\]/g,""),a=a.replace(/\(.*?\)/g,
""),a=a.replace(/\{.*?\}/g,""));var d=a,a=encodeURIComponent(a),t="https://translate.google.com.tw/#"+b.source_lang.value+"/"+b.target_lang.value+"/"+a,u=b.output_style.value;if(b.test_output.checked){var z=$(".google_trans_20140526 .test-output").show(),k=$("<div class='test-div panel panel-success'></div>").prependTo(z),p=new Date,l=p.getHours(),h=p.getMinutes(),A=p.getSeconds(),a=d,a=_googleTransUtils.str_replace("\n"," ",a);$("<div class='animated panel-heading togglable'></div>").html('<span class="glyphicon glyphicon-chevron-up" style="float:right"></span><span class="glyphicon glyphicon-chevron-down" style="float:right"></span><div class=\'heading-text\'><strong>'+
l+":"+h+":"+A+"</strong> "+a+"</div> ").click(function(){_googleTransUtils.toggle_panel(this)}).appendTo(k);_googleTransUtils.toggle_panel($(".google_trans_20140526 .input-div .panel-heading.togglable"));for(var q=$('<div style="display: inline-block; text-align:center"></div>'),r=$('<select class="form-control"></select>'),l=[["zh-TW","\u6b63\u9ad4\u4e2d\u6587"],["zh-CN","\u7b80\u4f53\u4e2d\u6587"],["ja","\u65e5\u672c\u8a9e"],["en","English"]],h=0;h<l.length;h++)r.append('<option value="'+l[h][0]+
'">'+l[h][1]+"</option>");r.attr("target_lang",b.target_lang.value).val(b.target_lang.value).change(function(){var a=$(this).parents(".test-div").find("textarea.source").val();$(this).parents(".test-div").find("textarea.note").addClass("wait-trans");var b=$("#transTempIframe")[0],b=b.contentWindow?b.contentWindow:b.contentDocument.defaultView,d=$(b.document.body);d.find("#source").html(a);d.find("#originalSource").val(a);b.setGoogleTransCookie(this.value);b.googleTranslateElementInit()}).appendTo(q);
"submit_authors"!==$(b).attr("submit_type")&&$('<button type="button" class="btn btn-success btn-block"></button>').html("\u8907\u88fd").click(function(){""===document.cookie&&(document.cookie="googtrans=/en/zh-TW");var a=[];$(this).parents(".test-div:first").find("textarea").each(function(b,d){a.push(d.value)});a=a.join("\n\n").trim();if(""!==a){var b=$("#copyInput");0===b.length&&(b=$('<textarea id="copyInput" style="position:absolute; top: -999px"></textarea>').appendTo("body"));b.val(a);b.eq(0).select();
document.execCommand("copy")}}).appendTo(q);var m=$("<textarea class='animated source panel-body'></textarea>").val(d).click(function(){this.select()}).appendTo(k),B=$("<textarea class='animated source panel-body note loading'></textarea>").click(function(){this.select()}).appendTo(k);"submit_authors"===$(b).attr("submit_type")&&k.addClass("submit_authors");q.appendTo(k);var C=function(){$(".input-div .panel-body").slideUp()};$(function(){m.autosize();B.autosize();m.focus();m.click();m.click(C);$(".google-trans-title").hide()});
b.window_open_output.checked&&setTimeout(function(){return r.change()},500)}else if("blank"===u)window.open(t,"_blank");else if("iframe"===u){var D=b.subject.value,E=(new Date).getTime();_googleTransUtils.create_iframe(t,E+" "+D).prependTo(".google_trans_20140526 .output")}}catch(v){console.log(v)}$(b.source).val("");return!1};
