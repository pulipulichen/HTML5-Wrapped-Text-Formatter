...
b.trim_citation.checked&&(a=a.replace(/\n\d+$/g,""),a=a.replace(/\n\d+ /g,"\n"));for(;-1<a.indexOf("  ");)a=a.split("  ").join(" ");for(;-1<a.indexOf("\n ");)a=a.split("\n ").join("\n");for(;-1<a.indexOf(". \n\n.");)a=a.split(". \n\n.").join("..");for(;-1<a.indexOf("\n\n\n\n");)a=a.split("\n\n\n\n").join("\n\n");a=a.replace(/[A-Z]\. \n\n/g,function(a){return a.split("\n").join("")});a=$.trim(a);if("submit_authors"===$(b).attr("submit_type")){var a=a.substr(0,a.indexOf(" (")).trim(),a=a.split(" and ").join(", "),
d=a.split(","),c=[];console.log(d);var l=2;1===d.length%2&&(l=1);for(var h=0,r=d.length;h<r;h+=l){var e=d[h];2===l&&(e=e+", "+d[h+1]);var e=e.split("&").join(""),m=e.indexOf(", ");if(-1<m)var t=e.substr(0,m).trim(),u=e.slice(m+1,e.length).trim(),e=t+"\t"+u;c.push(e)}a=c.join("\n")}var a=a.replace(/\[.*?\]/g,""),a=a.replace(/\(.*?\)/g,""),d=a=a.replace(/\{.*?\}/g,""),a=encodeURIComponent(a),p="https://translate.google.com.tw/#"+b.source_lang.value+"/"+b.target_lang.value+"/",a=p+a,f=b.output_style.value;
if(b.test_output.checked){var v=$(".google_trans_20140526 .test-output").show(),g=$("<div class='test-div panel panel-success'></div>").prependTo(v),n=new Date,w=n.getHours(),x=n.getMinutes(),y=n.getSeconds(),f=d,f=_googleTransUtils.str_replace("\n"," ",f);$("<div class='animated panel-heading togglable'></div>").html('<span class="glyphicon glyphicon-chevron-up" style="float:right"></span><span class="glyphicon glyphicon-chevron-down" style="float:right"></span><div class=\'heading-text\'><strong>'+
w+":"+x+":"+y+"</strong> "+f+"</div> ").click(function(){_googleTransUtils.toggle_panel(this)}).appendTo(g);_googleTransUtils.toggle_panel($(".google_trans_20140526 .input-div .panel-heading.togglable"));var z=$("<button type='button'></button>").html("\u7ffb\u8b6f").attr("pulipuli_base_url",p).click(function(){var a=$(this).attr("pulipuli_base_url")+$(this).next().val();window.open(a,"_blank","height=600,width=800,scrollbars=no")}).appendTo(g),k=$("<textarea class='animated source panel-body'></textarea>").val(d).click(function(){this.select()}).appendTo(g),
A=$("<textarea class='animated source panel-body note'></textarea>").click(function(){this.select()}).appendTo(g);"submit_authors"===$(b).attr("submit_type")&&g.addClass("submit_authors");var B=function(){$(".input-div .panel-body").slideUp()};$(function(){k.autosize();A.autosize();k.focus();k.click();k.click(B);$(".google-trans-title").hide()});b.window_open_output.checked&&setTimeout(function(){return z.click()},500)}else if("blank"===f)window.open(a,"_blank");else if("iframe"===f){var C=b.subject.value,
D=(new Date).getTime();_googleTransUtils.create_iframe(a,D+" "+C).prependTo(".google_trans_20140526 .output")}}catch(q){console.log(q)}$(b.source).val("");return!1};$(function(){$("#submitToGoogleTransForm").submit(function(b){return submitToGoogleTrans(this,b)});$("#submitToGoogleTransForm :submit").click(function(){$("#submitToGoogleTransForm").attr("submit_type",this.name)})});
